...
`validate_data` は前回スナップショットからの変化が妥当な範囲かを閾値で見る。
年を追加する更新（新しい貿易年、遡及取得した過去版の生産）では別の問いが要る。
すでに公開している数値が動いていないかである。`pipeline/invariance.py` がそれを見る。

    python -m pipeline.invariance --current <candidate.json> [--max-year 2024]

前回スナップショットに存在するキーはすべて同一で戻らなければならない。
concentration は `(hs_code, year)`、production は `(year, country)` でキー化し、
指数・集中度・報告総量・報告完全性・国別シェアと、査読済み生産行の各列を突き合わせる。
前回に無いキーは新規であり、ここでは見ない。差分があれば終了コード1を返す。
上流の遡及改訂は実在するが、それを採用するかは人間の判断であり、
更新処理が黙って飲み込んでよいものではないので、報告して止まる。

許容は相対1e-9のみで、これは触っていない式の中の加算順序が変わった場合のためにある。
使用時は必ずログとレポートの `tolerated` に残る。

この検査を通すうえで一つ承知しておくべき設計挙動がある。`coverage_pct` は報告完全性を
その段階の**全期間中央値**に対する相対値として出す。したがって系列に年が一つ加わると
分母が変わり、既に公開している全年の値が同時に動く。2025年を足したときは283691の
中央値が48から47に下がり、2017–2024年の8年がそれぞれ+2.1ポイント動いた。これは上流の
遡及改訂でも実装の誤りでもなく、指標の定義そのものである。公開しない年は分母に入れない
（段階の `end_year` を見る）が、公開する年は必ず入る。他の鉱物を追加するときも同じことが
起きるので、不変検査がこの項目を挙げたら、まず年が増えたかどうかを見ればよい。

既知の差分が1件ある。283691 / 2020 / CAN の `mirror_value` は6か国の輸入申告の和で、
その和が `9.02565` という丸め境界にちょうど乗る。浮動小数点の加算順序で最終ビットが
変わるため、`round(x, 4)` の結果が `9.0257` と `9.0256` に割れる。公開済みの値は前者、
現行コードが同じ生バイト列から再計算する値は後者である。指数・集中度・シェア率・
報告総量はいずれも動かず、量の差は0.0001 t（100 g）にとどまる。

宿題として、`process_trade.total` を加算順序に依存しない実装（`math.fsum` など）に
替える案がある。再現性を掲げるパイプラインで丸め境界の非決定性を残すのは弱点であり、
本来は直すべきである。ただし実施すれば公開値が動くため、データ側の作業とは
独立したコミットで、値が動くことを承知のうえで行う。

もう一件、同種の宿題がある。Comtradeの応答には `elapsedTime`（サーバ側の処理時間）が
含まれ、これは同じクエリを同じ日に取り直すだけで変わる。`archive.save` は本文全体の
ハッシュで同一性を判定するため、測定値がまったく同じ応答でも新しいアーカイブとして
保存される。2026-09-14の監査では48クエリすべてがこれに該当し、`data/raw/comtrade` が
約7MB増えたが、正規化後の行は48クエリとも完全に一致していた。content-address の
対象から `elapsedTime` を除く案があるが、過去のアーカイブの同一性判定が変わるため、
これも独立したコミットで扱う。

三件目は暫定年の表示である。`provisional_years` は段階キーで引くため、段階ではない
`headline_usd` に暫定年を付けられない（設定に書くと `unknown stages` で拒否される）。
2025年のheadlineは283691と282520の報告輸出額から作られ、どちらの段階も報告完全性が
平年を下回る年なのに、ページ上は暫定の印が付かない。仕組み側の課題であり、
段階キーの制約をどう緩めるかを決めてから着手する。

### USGS過去版からの鉱山生産の遡及取得

鉱山生産はMCS 2026版の2年分だけで始まった。2017–2023年は MCS 2019〜2025 の7版から
遡って取り込んでいる。原本は `data/raw/usgs/` に1ファイルとして保管する。全巻から
リチウム章を抜き出して連結したもので、記録しているsha256はその連結ファイルに対する
ものであり、どの巻に対するものでもない。版ごとの公式URL（全巻・章）は
`pipeline/lithium_usgs_sources.json` にある。なお本作業を行った環境からは
pubs.usgs.gov に到達できないため、URLはUSGSの公表命名に従って記録しただけで
再取得による確認は行っていない。抽出が実際に対象としたのは保管したバイト列である。

各データ年は、その年を報告する最新版から採る。MCS第N版はN-1年を推定値、N-2年を
改訂値として載せるので、最新版を採ることは確定値を採ることに等しい。どの行がどの版
由来かは `edition` 列と `source_url` 列に残り、`pipeline/tests/test_usgs_history.py`
がこの規則をCIで機械的に検査する（同テストは保管バイト列のsha256照合も行う。
いずれもネットワークに依存しない）。

| データ年 | 採用版 | 国別合計 vs 印字された世界計 |
|---|---|---|
| 2017 | MCS 2019 | −0.72% |
| 2018 | MCS 2020 | −0.11% |
| 2019 | MCS 2021 | +0.12% |
...
| 2022 | MCS 2024 | +0.31% |
| 2023 | MCS 2025 | −0.04% |
| 2024・2025 | MCS 2026 | +0.44% / −0.56% |

USGSは世界計を丸めるため残差は常に出る。取り込み時の許容は5%で、
`update_minerals.assemble` が同じ条件で改めて検査する。

抽出は `tools/extract_usgs_history.py` で行う。ページは「LITHIUM と
World Mine Production の両方を含む」ことで機械的に選び、本文は読まない。数値は
右揃えなのでspanの右端で列を決め、同一行は5pt以内で束ねる。脚注番号は本文9.96ptに対し
6.48ptなので、フォントサイズで数値と分離する。これを怠ると世界計の前に置かれた参照
番号が数字に融合し、69,000が869,000になる。PyMuPDFは `tools/requirements-extract.txt`
にだけ置き、`pipeline/requirements.txt` には入れない。公開経路はPDFを読まないし、
PyMuPDFはAGPL-3.0で本リポジトリはMITである。抽出の全記録（採用しなかった旧版の
推定値、除外した脚注spanの数、検算差）は `data/review/lithium-usgs-history/` に
JSONとCSVの両方で残している。
